...
import {state} from './state.js?v=v16-1-bottom-content-spacing-20260803-30';
import {fmt} from './format.js?v=v16-1-bottom-content-spacing-20260803-30';
import {minutes,pay,summary,daySummary} from './payroll.js?v=v16-1-bottom-content-spacing-20260803-30';
import {template} from './ui.js?v=v16-1-bottom-content-spacing-20260803-30';
...
function setupVisualViewportFix(){
 ...
 document.addEventListener('focusout',()=>{
  clearTimeout(timer);
  timer=setTimeout(()=>document.documentElement.classList.remove('keyboard-open'),250);
 });

 document.addEventListener('change',event=>{
  if(event.target?.matches?.('select,input[type="date"],input[type="time"],input[type="color"]')){
   clearTimeout(timer);
   timer=setTimeout(()=>document.documentElement.classList.remove('keyboard-open'),100);
  }
 });
...
function backupPayload(){return {backupSchema:2,appVersion:'v16.1 Bottom Content Spacing',exportedAt:new Date().toISOString(),profiles:storage.exportStore()}}
